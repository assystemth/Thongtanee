...
class Report_model extends CI_Model
{
    ...
    public function find_most_viewed_table()
    {
        ...
        UNION ALL
        SELECT 'ดาวน์โหลดน์แบบฟอร์ม' AS table_name, SUM(loadform_view) AS total_views FROM tbl_loadform
        UNION ALL
        ...
        UNION ALL
        SELECT 'ข้อบัญญัติงบประมาณ' AS table_name, SUM(canon_bgps_view) AS total_views FROM tbl_canon_bgps
        UNION ALL
        SELECT 'การควบคุมกิจการที่เป็นอันตรายต่อสุขภาพ' AS table_name, SUM(canon_chh_view) AS total_views FROM tbl_canon_chh
        UNION ALL
        SELECT 'ข้อบัญญัติการติดตั้งระบบบำบัดน้ำเสียในอาคาร' AS table_name, SUM(canon_ritw_view) AS total_views FROM tbl_canon_ritw
        UNION ALL
        SELECT 'ข้อบัญญัติตลาด' AS table_name, SUM(canon_market_view) AS total_views FROM tbl_canon_market
        UNION ALL
        SELECT 'ข้อบัญญัติการจัดการสิ่งปฏิกูลและมูลฝอย' AS table_name, SUM(canon_rmwp_view) AS total_views FROM tbl_canon_rmwp
        UNION ALL
        SELECT 'ข้อบัญญัติหลักเกณฑ์การคัดมูลฝอย' AS table_name, SUM(canon_rcsp_view) AS total_views FROM tbl_canon_rcsp
        UNION ALL
        SELECT 'ข้อบัญญัติการควบคุมการเลี้ยงหรือปล่อยสุนัขและแมว' AS table_name, SUM(canon_rcp_view) AS total_views FROM tbl_canon_rcp
        UNION ALL
