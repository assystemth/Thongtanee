...
class Pbsv_ems_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        if (isset($_FILES['pbsv_ems_file_pdf'])) {
            foreach ($_FILES['pbsv_ems_file_pdf']['name'] as $index => $name) {
                if (isset($_FILES['pbsv_ems_file_pdf']['size'][$index])) {
                    $total_space_required += $_FILES['pbsv_ems_file_pdf']['size'][$index];
                }
            }
        }
        ...
        // ตรวจสอบว่ามีการอัปโหลดรูปภาพเพิ่มเติมหรือไม่
        if (!empty($_FILES['pbsv_ems_file_pdf']['name'][0])) {
            foreach ($_FILES['pbsv_ems_file_pdf']['name'] as $index => $name) {
                $_FILES['pbsv_ems_file_pdf_multiple']['name'] = $name;
                $_FILES['pbsv_ems_file_pdf_multiple']['type'] = $_FILES['pbsv_ems_file_pdf']['type'][$index];
                $_FILES['pbsv_ems_file_pdf_multiple']['tmp_name'] = $_FILES['pbsv_ems_file_pdf']['tmp_name'][$index];
                $_FILES['pbsv_ems_file_pdf_multiple']['error'] = $_FILES['pbsv_ems_file_pdf']['error'][$index];
                $_FILES['pbsv_ems_file_pdf_multiple']['size'] = $_FILES['pbsv_ems_file_pdf']['size'][$index];

                if ($this->pdf_upload->do_upload('pbsv_ems_file_pdf_multiple')) {
                    $upload_data = $this->pdf_upload->data();
                    $pdf_data[] = array(
                        'pbsv_ems_file_ref_id' => $pbsv_ems_id,
                        'pbsv_ems_file_pdf' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_pbsv_ems_file', $pdf_data);
        }
    ...
    {
        $this->db->select('pbsv_ems_file_pdf');
    ...
    public function read_file($pbsv_ems_id)
    {
        $this->db->where('pbsv_ems_file_ref_id', $pbsv_ems_id);
    ...
    public function read_img($pbsv_ems_id)
    {
        ...
        $this->db->order_by('pbsv_ems_img_id', 'DESC');
    ...
    public function del_pdf($file_id)
    {
        ...
        $this->db->select('pbsv_ems_file_pdf');
        $this->db->where('pbsv_ems_file_id', $file_id);
        $query = $this->db->get('tbl_pbsv_ems_file');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->pbsv_ems_file_pdf;
    ...
    public function del_pbsv_ems_pdf($pbsv_ems_id)
    {
        ...
        foreach ($files as $files) {
            $file_path = './docs/file/' . $files->pbsv_ems_file_pdf;
            if (file_exists($file_path)) {
                unlink($file_path);
            }
        }
