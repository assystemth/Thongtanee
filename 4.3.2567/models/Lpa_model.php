...
class Lpa_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        if (isset($_FILES['lpa_file_pdf'])) {
            foreach ($_FILES['lpa_file_pdf']['name'] as $index => $name) {
                if (isset($_FILES['lpa_file_pdf']['size'][$index])) {
                    $total_space_required += $_FILES['lpa_file_pdf']['size'][$index];
                }
            }
        }
        ...
        // ตรวจสอบว่ามีการอัปโหลดรูปภาพเพิ่มเติมหรือไม่
        if (!empty($_FILES['lpa_file_pdf']['name'][0])) {
            foreach ($_FILES['lpa_file_pdf']['name'] as $index => $name) {
                $_FILES['lpa_file_pdf_multiple']['name'] = $name;
                $_FILES['lpa_file_pdf_multiple']['type'] = $_FILES['lpa_file_pdf']['type'][$index];
                $_FILES['lpa_file_pdf_multiple']['tmp_name'] = $_FILES['lpa_file_pdf']['tmp_name'][$index];
                $_FILES['lpa_file_pdf_multiple']['error'] = $_FILES['lpa_file_pdf']['error'][$index];
                $_FILES['lpa_file_pdf_multiple']['size'] = $_FILES['lpa_file_pdf']['size'][$index];

                if ($this->pdf_upload->do_upload('lpa_file_pdf_multiple')) {
                    $upload_data = $this->pdf_upload->data();
                    $pdf_data[] = array(
                        'lpa_file_ref_id' => $lpa_id,
                        'lpa_file_pdf' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_lpa_file', $pdf_data);
        }
    ...
    public function read_file($lpa_id)
    {
        $this->db->where('lpa_file_ref_id', $lpa_id);
    ...
    public function read_img($lpa_id)
    {
        $this->db->where('lpa_img_ref_id', $lpa_id);
        $this->db->order_by('lpa_img_id', 'DESC');
    ...
    public function del_pdf($file_id)
    {
        ...
        $this->db->select('lpa_file_pdf');
        $this->db->where('lpa_file_id', $file_id);
        $query = $this->db->get('tbl_lpa_file');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->lpa_file_pdf;
    ...
    public function del_lpa_pdf($lpa_id)
    {
        ...
        foreach ($files as $files) {
            $file_path = './docs/file/' . $files->lpa_file_pdf;
            if (file_exists($file_path)) {
                unlink($file_path);
            }
        }
