...
class P_rpobuy_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        if (isset($_FILES['p_rpobuy_file_pdf'])) {
            foreach ($_FILES['p_rpobuy_file_pdf']['name'] as $index => $name) {
                if (isset($_FILES['p_rpobuy_file_pdf']['size'][$index])) {
                    $total_space_required += $_FILES['p_rpobuy_file_pdf']['size'][$index];
                }
            }
        }
        ...
        // ตรวจสอบว่ามีการอัปโหลดรูปภาพเพิ่มเติมหรือไม่
        if (!empty($_FILES['p_rpobuy_file_pdf']['name'][0])) {
            foreach ($_FILES['p_rpobuy_file_pdf']['name'] as $index => $name) {
                $_FILES['p_rpobuy_file_pdf_multiple']['name'] = $name;
                $_FILES['p_rpobuy_file_pdf_multiple']['type'] = $_FILES['p_rpobuy_file_pdf']['type'][$index];
                $_FILES['p_rpobuy_file_pdf_multiple']['tmp_name'] = $_FILES['p_rpobuy_file_pdf']['tmp_name'][$index];
                $_FILES['p_rpobuy_file_pdf_multiple']['error'] = $_FILES['p_rpobuy_file_pdf']['error'][$index];
                $_FILES['p_rpobuy_file_pdf_multiple']['size'] = $_FILES['p_rpobuy_file_pdf']['size'][$index];

                if ($this->pdf_upload->do_upload('p_rpobuy_file_pdf_multiple')) {
                    $upload_data = $this->pdf_upload->data();
                    $pdf_data[] = array(
                        'p_rpobuy_file_ref_id' => $p_rpobuy_id,
                        'p_rpobuy_file_pdf' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_p_rpobuy_file', $pdf_data);
        }
    ...
    {
        $this->db->select('p_rpobuy_file_pdf');
    ...
    public function read_file($p_rpobuy_id)
    {
        $this->db->where('p_rpobuy_file_ref_id', $p_rpobuy_id);
    ...
    public function read_img($p_rpobuy_id)
    {
        ...
        $this->db->order_by('p_rpobuy_img_id', 'DESC');
    ...
    public function del_pdf($file_id)
    {
        ...
        $this->db->select('p_rpobuy_file_pdf');
        $this->db->where('p_rpobuy_file_id', $file_id);
        $query = $this->db->get('tbl_p_rpobuy_file');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->p_rpobuy_file_pdf;
    ...
    {
        // ดึงชื่อไฟล์ PDF จากฐานข้อมูลโดยใช้ $file_id
        $this->db->select('p_rpobuy_img_img');
    ...
    public function del_p_rpobuy_pdf($p_rpobuy_id)
    {
        ...
        foreach ($files as $files) {
            $file_path = './docs/file/' . $files->p_rpobuy_file_pdf;
            if (file_exists($file_path)) {
                unlink($file_path);
            }
        }
    ...
    public function update_p_rpobuy_status()
    {
        ...
        if ($this->input->post()) {
            ...
            $p_rpobuytatus = $this->input->post('new_status'); // รับค่าใหม่จาก switch checkbox

            // ทำการอัพเดตค่าในตาราง tbl_p_rpobuy ในฐานข้อมูลของคุณ
            $data = array(
                'p_rpobuy_status' => $p_rpobuytatus
            );
