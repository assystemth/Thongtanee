...
class Operation_po_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        if (isset($_FILES['operation_po_pdf_pdf'])) {
            foreach ($_FILES['operation_po_pdf_pdf']['name'] as $index => $name) {
                if (isset($_FILES['operation_po_pdf_pdf']['size'][$index])) {
                    $total_space_required += $_FILES['operation_po_pdf_pdf']['size'][$index];
                }
            }
        }

        // ตรวจสอบว่ามีข้อมูลไฟล์ doc หรือไม่
        if (isset($_FILES['operation_po_file_doc'])) {
            foreach ($_FILES['operation_po_file_doc']['name'] as $index => $name) {
                if (isset($_FILES['operation_po_file_doc']['size'][$index])) {
                    $total_space_required += $_FILES['operation_po_file_doc']['size'][$index];
                }
            }
        }
        ...
        if (!empty($_FILES['operation_po_pdf_pdf']['name'][0])) {
            foreach ($_FILES['operation_po_pdf_pdf']['name'] as $index => $name) {
                $_FILES['operation_po_pdf_pdf_multiple']['name'] = $name;
                $_FILES['operation_po_pdf_pdf_multiple']['type'] = $_FILES['operation_po_pdf_pdf']['type'][$index];
                $_FILES['operation_po_pdf_pdf_multiple']['tmp_name'] = $_FILES['operation_po_pdf_pdf']['tmp_name'][$index];
                $_FILES['operation_po_pdf_pdf_multiple']['error'] = $_FILES['operation_po_pdf_pdf']['error'][$index];
                $_FILES['operation_po_pdf_pdf_multiple']['size'] = $_FILES['operation_po_pdf_pdf']['size'][$index];

                if ($this->pdf_upload->do_upload('operation_po_pdf_pdf_multiple')) {
                    $upload_data = $this->pdf_upload->data();
                    $pdf_data[] = array(
                        'operation_po_pdf_ref_id' => $operation_po_id,
                        'operation_po_pdf_pdf' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_operation_po_pdf', $pdf_data);
        }

        $doc_data = array();

        // ตรวจสอบว่ามีการอัปโหลดไฟล์Docเพิ่มเติมหรือไม่
        if (!empty($_FILES['operation_po_file_doc']['name'][0])) {
            foreach ($_FILES['operation_po_file_doc']['name'] as $index => $name) {
                $_FILES['operation_po_file_doc_multiple']['name'] = $name;
                $_FILES['operation_po_file_doc_multiple']['type'] = $_FILES['operation_po_file_doc']['type'][$index];
                $_FILES['operation_po_file_doc_multiple']['tmp_name'] = $_FILES['operation_po_file_doc']['tmp_name'][$index];
                $_FILES['operation_po_file_doc_multiple']['error'] = $_FILES['operation_po_file_doc']['error'][$index];
                $_FILES['operation_po_file_doc_multiple']['size'] = $_FILES['operation_po_file_doc']['size'][$index];

                if ($this->doc_upload->do_upload('operation_po_file_doc_multiple')) {
                    $upload_data = $this->doc_upload->data();
                    $doc_data[] = array(
                        'operation_po_file_ref_id' => $operation_po_id,
                        'operation_po_file_doc' => $upload_data['file_name']
                    );
                }
            }
            $this->db->insert_batch('tbl_operation_po_file', $doc_data);
        }
    ...
    {
        $this->db->select('operation_po_pdf_pdf');
        $this->db->from('tbl_operation_po_pdf');
        $this->db->where('operation_po_pdf_ref_id', $operation_po_id);
        return $this->db->get()->result();
    }
    public function list_all_doc($operation_po_id)
    {
        $this->db->select('operation_po_file_doc');
    ...
    public function read_pdf($operation_po_id)
    {
        $this->db->where('operation_po_pdf_ref_id', $operation_po_id);
        $this->db->order_by('operation_po_pdf_id', 'DESC');
        $query = $this->db->get('tbl_operation_po_pdf');
        return $query->result();
    }
    public function read_doc($operation_po_id)
    {
        $this->db->where('operation_po_file_ref_id', $operation_po_id);
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('operation_po_pdf_pdf');
        $this->db->where('operation_po_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_operation_po_pdf');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->operation_po_pdf_pdf;
        ...
        $this->db->where('operation_po_pdf_id', $pdf_id);
        $this->db->delete('tbl_operation_po_pdf');
        $this->space_model->update_server_current();
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('operation_po_file_doc');
        $this->db->where('operation_po_file_id', $doc_id);
        $query = $this->db->get('tbl_operation_po_file');
        $file_data = $query->row();

        // ลบไฟล์จากแหล่งที่เก็บไฟล์ (อาจต้องใช้ unlink หรือวิธีอื่น)
        $file_path = './docs/file/' . $file_data->operation_po_file_doc;
        if (file_exists($file_path)) {
            unlink($file_path);
        }
    ...
    public function del_operation_po_pdf($operation_po_id)
    {
        ...
        $files = $this->db->get_where('tbl_operation_po_pdf', array('operation_po_pdf_ref_id' => $operation_po_id))->result();

        // ลบ pdf จากตาราง tbl_operation_po_pdf
        $this->db->where('operation_po_pdf_ref_id', $operation_po_id);
        $this->db->delete('tbl_operation_po_pdf');

        // ลบไฟล์ pdf ที่เกี่ยวข้อง
        foreach ($files as $files) {
            $file_path = './docs/file/' . $files->operation_po_pdf_pdf;
            if (file_exists($file_path)) {
                unlink($file_path);
            }
        }
    ...
    public function del_operation_po_doc($operation_po_id)
    {
        ...
        // ลบ doc จากตาราง tbl_operation_po_file
        $this->db->where('operation_po_file_ref_id', $operation_po_id);
        $this->db->delete('tbl_operation_po_file');

        // ลบไฟล์ doc ที่เกี่ยวข้อง
        foreach ($files as $files) {
            $file_path = './docs/file/' . $files->operation_po_file_doc;
            if (file_exists($file_path)) {
                unlink($file_path);
            }
        }
    ...
    public function operation_po_frontend_list()
    {
        $this->db->select('*');
        $this->db->from('tbl_operation_po');
        $this->db->where('tbl_operation_po.operation_po_status', 'show');
